...
void Model::DrawM() {
	...
	for (int i = 0; i < nface; i++) {
		...
		for (int j = 0; j < 3; j++) {
			glNormal3dv(V[F[i].tri[j]].N); //Graficar normales para cada vertice
...
// Metodos y variables de la clase Hadouken
Hadouken::Hadouken(std::string name) {
	ReadM3D(name);
}

void Hadouken::Idle() {
	float mat_dif[] = { (float)(0.0), 0.0, 1.0, 1.0 };
	DrawM(mat_dif);
}
void Hadouken::RealizarMovimiento(bool& direccion, float& posicion, Hadouken* blue) {
	if (direccion) {
		blue->Idle();
		posicion += 0.5; // Velocidad de mov.
